...
public class FtsToBfmConverter<F extends Feature<F>> extends AbstractFtsConverter<F, BehavioralFeatureModel> {

    private BehavioralFeatureModelFactory factory;

    public FtsToBfmConverter(FeatureModel<F> fm, FeaturedTransitionSystem fts) {
        super(LoggerFactory.getLogger(FtsToBfmConverter.class), fm, fts);
    }
    ...
    protected void addEventToFactory(Event event, FExpression fexpr) {
        ...
        F root = fm.getRootFeature();
        BehavioralFeature bf = factory.getFeature(root.getFeatureName());
        factory.addEvent(bf, event.getName(), event.getAction(), fexpr);
    }
    ...
    protected void addCausalityToFactory(CausalityRelation relation) {
        factory.addCausality(fm.getRootFeature().getFeatureName(), relation);
    }

    @Override
    protected BehavioralFeatureModel buildResult() {
