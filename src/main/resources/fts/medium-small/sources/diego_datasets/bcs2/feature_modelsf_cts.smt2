(define-sort Feature () Bool)
(declare-const BCS Feature)
(declare-const HMI Feature)
(declare-const LED Feature)
(declare-const LEDAS Feature)
(declare-const LEDFP Feature)
(declare-const LEDCLS Feature)
(declare-const LEDPW Feature)
(declare-const LEDEM Feature)
(declare-const LEDH Feature)
(declare-const DS Feature)
(declare-const PW Feature)
(declare-const FP Feature)
(declare-const WIN Feature)
(declare-const ManPW Feature)
(declare-const AutPW Feature)
(declare-const EM Feature)
(declare-const ELE Feature)
(declare-const HEAT Feature)
(declare-const SEC Feature)
(declare-const AS Feature)
(declare-const IM Feature)
(declare-const CLS Feature)
(declare-const AL Feature)
(declare-const RCK Feature)
(declare-const CAS Feature)
(declare-const SF Feature)
(declare-const CAP Feature)
(declare-const AEM Feature)

(assert BCS)
(assert (and
   (=> HMI BCS)
   (=> LED HMI)
   (= (or LEDAS  LEDFP  LEDCLS  LEDPW  LEDEM  LEDH ) LED)
   (= DS BCS)
   (= PW DS)
   (= FP PW)
   (= WIN PW)
   (= (or ManPW  AutPW ) WIN)
   (not (and ManPW AutPW))
   (=> EM DS)
   (= ELE EM)
   (=> HEAT EM)
   (= SEC BCS)
   (=> AS SEC)
   (=> IM AS)
   (=> CLS SEC)
   (=> AL CLS)
   (=> RCK SEC)
   (=> CAS RCK)
   (=> SF RCK)
   (=> CAP RCK)
   (=> AEM RCK)
))

(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AL CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AL CLS))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true AL CLS))
(check-sat)
(pop)
(push)
(assert (and true AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AL CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and true AL CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW true))
(check-sat)
(pop)
(push)
(assert (and true AutPW true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true true true))
(check-sat)
(pop)
(push)
(assert (and true AL CLS))
(check-sat)
(pop)
(push)
(assert (and true AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and true ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true true))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true true))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true true))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true true))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true true))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true true))
(check-sat)
(pop)
(push)
(assert (and CLS ManPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS AutPW CLS))
(check-sat)
(pop)
(push)
(assert (and CLS true CLS))
(check-sat)
(pop)
